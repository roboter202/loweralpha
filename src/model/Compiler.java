...
public class Compiler {
	...
	private void checkAddresses(Token t)
	{
		...
		for(int i = 0; i < 1; i++)
		{
			...
			else if(t.addrs[i].charAt(0) == 'α')
			{
				if(registerLabelMap.get(t.addrs[i]) == null)
				{
					registerLabelMap.put(t.addrs[i], registerLabelIndex); //fügt das Pseudonym ggf hinzu
					registerLabelIndex++;
				}
			}
			else if(t.addrs[i].charAt(0) == 'ρ') //Speicherzugriff
			{
				if(t.addrs[i].length() > 3 && t.addrs[i].charAt(2) == 'ρ')//indirekter Speicherzugriff durch Speicherstelle
				{ 
					String addr = convertMemoryAddress(t.addrs[i]);
				...
				else if(t.addrs[i].length() > 3 && t.addrs[i].charAt(2) == 'α')//indirekter Speicherzugriff durch Regsiter
				{
					String addr = convertMemoryAddress(t.addrs[i]);
